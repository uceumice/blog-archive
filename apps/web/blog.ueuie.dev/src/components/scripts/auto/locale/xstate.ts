...
import { Lang, parse } from "~/utils/i18n";
...
export const xstate = createMachine({
    ...
    states: {
        ...
        getting_locale_by_cf_headers: {
            invoke: {
                ...
                onDone: {
                    target: 'idle',
                    actions: assign({ selected: (_, { data }) => data as Lang }),
                },
                onError: {
    ...
    actions: {
        "get.locale.by.url.prefix": assign(() => {
            ...
            const locale = parse.lang(prefix);
        ...
        "sync.to.local-storage": (ctx) => {
            ...
            if (prefered_ === null || parse.lang(prefered_) === null) {
    ...
    services: {
        "fetch.cf": async () => {
            const response = await fetch(`${import.meta.env.DEV ? 'http://localhost:3000' : import.meta.env.SITE}/apis_/locale`).then(res => res.json());
            if (!response) throw Error();
            if (!parse.lang(response)) throw Error();
            return response as Lang;
        }
    },
    guards: {
        "empty": ({ selected }) => {
